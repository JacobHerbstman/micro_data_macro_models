...
\begin{enumerate}[label=(\roman*)]
    ...
    For a full test, I would merge the ACS repeated cross-sections with local labor-market and housing data, so these mechanisms can be tested in the same regression framework. 
...
\begin{enumerate}[label=(\roman*)]
    ...
    The 2010 and 2019 versions use the same 2006 top-75 list and the same definitions. 
...
\begin{enumerate}[label=(\roman*)]
    ...
    New Orleans tops the gross-flow table because Katrina outflows were still enormous in 2006. Baton Rouge is also likely affected by Katrina-related displacement, so I do not interpret its 2006 net inflow as a normal high-growth-metro result. New Orleans' gross flow rate is 0.236, but its net flow rate is $-0.193$. Outside the Katrina-affected entries, the 2006 lists mostly pick up high-turnover and growing Sun Belt or coastal metros.
\end{enumerate}
...
\begin{enumerate}[label=(\roman*)]
    ...
    By 2019, New Orleans dominates because 2006 is an abnormal base year. The largest increases include Dayton, Allentown, and Fresno, where net flows improved relative to the 2006 baseline. On the negative side, the list includes former growth or high-churn metros as well as older industrial metros.
\end{enumerate}
